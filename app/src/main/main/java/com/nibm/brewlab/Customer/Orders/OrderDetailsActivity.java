...
public class OrderDetailsActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        orderDocRef = FirebaseFirestore.getInstance().collection("Orders").document(orderId);
        loadOrder();
    ...
    private void loadOrder() {
        ...
        orderListener = orderDocRef.addSnapshotListener((snapshot, error) -> {
            ...
            CustomerOrder order = CustomerOrder.fromFirestore(snapshot);
            currentOrder = order;
            ...
            boolean hasDeliveryPerson = order.getDeliveryPersonName() != null && !order.getDeliveryPersonName().isEmpty();
            boolean outForDelivery = "Out for Delivery".equalsIgnoreCase(order.getStatus())
                    || "Picked Up".equalsIgnoreCase(order.getStage());
    ...
    private void reorderSelected() {
        ...
        com.google.firebase.firestore.CollectionReference productsRef =
        ...
        for (int i = 0; i < currentItems.size(); i++) {
            ...
            productsRef.document(item.getProductId()).get().addOnSuccessListener(snapshot -> {
    ...
    private String buildTrackingText(CustomerOrder order) {
        ...
        // A delivery person can be assigned/accept the order before the
        // shop even marks it "Out for Delivery" - show that as soon as
        // it happens, using whichever field the Admin/Delivery apps set.
        if ("Accepted".equalsIgnoreCase(stage)) {
            return "Accepted! A delivery person is on the way to pick up your order.";
        }
